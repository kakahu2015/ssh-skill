...
expect_success \
  ...
  bash "$ROOT/scripts/agent_gate.sh" --decision "$MOCK_OK_DECISION" --policy "$ROOT/autonomy.example.yaml" --execute
...
expect_failure_contains \
  ...
  "verification_failed" \
  bash "$ROOT/scripts/agent_gate.sh" --decision "$MOCK_VERIFY_FAIL_DECISION" --policy "$ROOT/autonomy.example.yaml" --execute --rollback-on-failed-verification
...
expect_failure_contains \
  ...
  "action_failed" \
  bash "$ROOT/scripts/agent_gate.sh" --decision "$MOCK_ACTION_FAIL_DECISION" --policy "$ROOT/autonomy.example.yaml" --execute
